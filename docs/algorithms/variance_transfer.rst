...
For layers of the form :math:`y_l = W_l x_l + b_l`. Assuming that each element of :math:`x_l` and :math:`W_l` are independently distributed, and the biases are zeroed. The variance of the preactivations :math:`y_l` is given by
...
.. table:: Maximal Update Parameterisation (:math:`\mu P`) vs the PyTorch Parameterisation in brackets (if different). See Table 3 in :cite:p:`yang_tensor_2021` for more details.
    :align: center
