...
study :cite:p:`du_stacking_2024` demonstrates that depthwise
stacking achieves 54.6% speedup when training a 7B parameter model on
750B tokens. Compound growth :cite:p:`gu_bert_2021`, expanding
depth, width, and sequence length simultaneously, achieves 73.6-82.2%
speedup on BERT. At smaller scales, MSG :cite:p:`yao_msg_2024`
achieves 2.2\ :math:`\times` speedup on BERT-Large, and
AutoProg :cite:p:`li_autoprog_2022` reaches 85.1% speedup on
...
scale :cite:p:`du_stacking_2024`—a finding that challenges the
...
contradictory. MSG :cite:p:`yao_msg_2024` achieves strong
results using strict function preservation, while
AutoProg :cite:p:`li_autoprog_2022` finds that function
preservation *harms* Vision Transformer performance (:math:`-3.21\%`).
Most strikingly, simple
stacking :cite:p:`du_stacking_2024`—which violates function
...
toward reasoning* :cite:p:`saunshi_inductive_2024`: models
